...
public class TransactionalMessageServiceImpl implements TransactionalMessageService {
    ...
    // 在执行事务消息回查之前，竟然在此把该消息存储在commitlog文件，新的消息设置最新的物理偏移量。
    // 为什么需要这样处理呢？主要是因为下文的发送事务消息是异步处理的，无法立刻知道其处理结果，
    // 为了避免简化prepare消息队列和处理队列的消息消费进度处理，先存储，然后消费进度向前推动，
    // 重复发送的消息在事务回查之前会判断是否处理过。另外一个目的就是需要修改消息的检查次数，
    // RocketMQ的存储设计采用顺序写，去修改已存储的消息，其性能无法高性能
    ...
    public void check(long transactionTimeout, int transactionCheckMax,
        ...
        try {
            ...
            for (MessageQueue messageQueue : msgQueues) {
                ...
                long i = halfOffset;
                while (true) {
                    //这段代码大家应该并不陌生，RocketMQ处理任务的一个通用处理逻辑就是为每个任务一次只分配某个固定时长，超过该时长则需等待下次任务调度。
                    //RocketMQ为待检测主题RMQ_SYS_TRANS_HALF_TOPIC的每个队列做事务状态回查，一次最多不超过60秒，目前该值不可配置。
                    ...
                    if (removeMap.containsKey(i)) {
                        ...
                        long valueOfCurrentMinusBorn = System.currentTimeMillis() - msgExt.getBornTimestamp();
                        //transactionTimeout ： 事务消息的超时时间，这个时间是从OP拉取的消息的最后一条消息的存储时间与check方法开始的时间，如果时间差超过了transactionTimeout，
                        //就算时间小于checkImmunityTime时间，也发送事务回查指令。
                        //checkImmunityTime ： 立即检测事务消息的时间，其设计的意义是，应用程序在发送事务消息后，事务不会马上提交，该时间就是假设事务消息发送成功后，
                        ...
                        if (isNeedCheck) {
                            //如果需要发送事务状态回查消息，则先将消息再次发送到RMQ_SYS_TRANS_HALF_TOPIC主题中，发送成功则返回true，否则返回false，这里还有一个实现关键点
                            if (!putBackHalfMsgQueue(msgExt, i)) {
                                continue;
                            }

                            //发送具体的事务回查命令，使用线程池来异步发送回查消息，为了回查消费进度保存的简化，只要发送了回查消息，当前回查进度会向前推动
                            //broker端 ： 最终调用AbstractTransactionalMessageCheckListener.sendCheckMessage，向producer发送消息
                            //producer端 ： 事务回查命令的最终处理者为ClientRemotingProcessor的processRequest方法，最终将任务提交到TransactionMQProducer的线程池中执行，最终调用应用程序实现的Transaction Listener的checkLocalTransaction方法，返回事务状态。
