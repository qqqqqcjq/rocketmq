...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
    ...
    // 注意：如果从消息进度服务OffsetStore读取到MessageQueue中的偏移量不小于0，则使用读取到的偏移量，只有在读到的偏移量小于0时，上述策略才会生效。
     ...
     */
    //消费者最大线程数，由于消费者线程池使用无界队列(当核心线程数满了后，任务优先进入等待队列。如果等待队列也满了后，才会去创建新的非核心线程 )，
    //故消费者线程个数其实最多只有consumeThreadMin个
     ...
     */
    //消息并发消费时一次消费消息条数，通俗点说就是每次传入MessageListtener#consumeMessage中的消息条数。也就是给每一个消费者线程一次传入的消息数量
     ...
     */
    //延迟将该队列的消息提交到消费者线程的等待时间，默认延迟1s。可用于流量控制等
    ...
    /**
     * 订阅一个topic的几个标签， 类似于下面的筛选功能
     * select * from t where key = 'topic_order' tag in('a', 'c')
     * select * from t where key = 'topic_pay' tag in('a', 'b')
     *
