...
public class DefaultMQPushConsumerImpl implements MQConsumerInner {
    ...
    public void pullMessage(final PullRequest pullRequest) {
        ...
        //Step1：从PullRequest中获取ProcessQueue，如果处理队列当前状态未被丢弃，
        // 则更新ProcessQueue的lastPullTimestamp为当前时间戳；如果当前消费者被挂起，
        // 则将拉取任务延迟1s再次放入到PullMessageService的拉取任务队列中，结束本次消息拉取。
        ...
        //1）消息处理总数，如果ProcessQueue当前处理的消息条数超过了pullThresholdForQueue=1000将触发流控，放弃本次拉取任务，
        ...
        if (!this.consumeOrderly) {
            //并发消费
            //2 ）ProcessQueue中队列最大偏移量与最小偏离量的间距，不能超过consumeConcurrentlyMaxSpan，否则触发流控，每触发1000次输出提示语
        ...
        //Step4：构建消息拉取系统标记
        //下面让我们一一来介绍PullSysFlag的枚举值含义。sysFlag用不同的位来标记
        ...
        try {
            //Step5：调用PullAPIWrapper.pullKernelImpl方法后与服务端交互，调用pullKernelImpl方法之前先了解一下其参数含义。
